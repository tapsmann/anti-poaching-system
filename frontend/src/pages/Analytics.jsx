...
const Analytics = () => {
  ...
  const metrics = [
    ...
    ['Average response', `${analytics.avg_response_time_mins} min`],
    ['Active patrols', analytics.active_patrols],
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500">Backend incident history and Python model predictions</p>
        </div>
        <button onClick={exportJson} className="btn-gold flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {metrics.map(([label, value]) => (
          <div className="card-zim" key={label}>
            ...
            <p className="text-3xl font-bold text-zim-800 mt-1">{value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <section className="card-zim">
          ...
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={analytics.poaching_trends}>
        ...
        <section className="card-zim">
          ...
          <IncidentMap incidents={analytics.recent_incidents} hotspots={heatmap} height="18rem" />
          <ol className="mt-4 space-y-1 text-sm text-gray-600">
            {hotspotRows.map((spot, index) => (
              <li key={`${spot.lat}-${spot.lng}`}>
                <span className="font-medium text-red-700">#{index + 1} {Math.round(spot.risk_score)}%</span> risk at {spot.lat.toFixed(2)}, {spot.lng.toFixed(2)}
              </li>
            ))}
          </ol>
        </section>
