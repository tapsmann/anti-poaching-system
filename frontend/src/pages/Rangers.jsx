...
const Rangers = () => {
  ...
  const load = async () => {
    try {
      setError('');
      const { data } = await rangersApi.getAll();
      setRangers(data || []);
    } catch (e) {
      setError(e.response?.data?.detail || 'Unable to load rangers.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {visible.map((ranger) => (
          <article key={ranger.id} className="card-zim">
            <div className="flex gap-4">
              ...
              <div>
                ...
                <p className="mt-2 text-xs flex items-center gap-1">
                  ...
                  <Wifi size={12} className="ml-2" /> {ranger.specialization?.replace('_', ' ') || 'General'}
                </p>
              </div>
            </div>
          </article>
        ))}
        {!visible.length && <p className="text-gray-500">No rangers found.</p>}
      </div>
    </div>
  );
};
